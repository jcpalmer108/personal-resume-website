...
describe("Button", () => {
  test("renders if required and all optional params are passed in", () => {
    ...
    expect(
      screen.getByRole("link", { name: `${upperCaseLabel} arrow` })
    ).toHaveAttribute("href", optional.url);
    expect(screen.getByTestId("Button")).toHaveTextContent(upperCaseLabel);
    expect(screen.getByRole("img", { name: "arrow" })).toHaveAttribute(
      "src",
      "arrow-right.svg"
    );
  });
  ...
  test("renders if label and onClick params are passed in", () => {
    ...
    expect(screen.getByTestId("Button")).toMatchSnapshot();
    expect(
      screen.queryByRole("link", { name: `${upperCaseLabel} arrow` })
    ).toBeFalsy();
    expect(screen.getByTestId("Button")).toHaveTextContent(upperCaseLabel);
    expect(screen.getByRole("img", { name: "arrow" })).toHaveAttribute(
      "src",
