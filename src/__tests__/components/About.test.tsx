...
describe("About", () => {
  test("renders if only required params are passed in", () => {
    ...
    required.description.forEach((item, index) => {
      expect(screen.getByTestId(`Description ${index + 1}`)).toHaveTextContent(
        item
      );
    });
    expect(screen.getByTestId("Schooling").childNodes).toHaveLength(
      required.subSection.education.length + 1
    );
    required.subSection.education.forEach((item, index) => {
      expect(
        screen.getByTestId(`School ${index + 1}`).childNodes[0]
      ).toHaveTextContent(item.school);
      expect(
        screen.getByTestId(`School ${index + 1}`).childNodes[1]
      ).toHaveTextContent(
